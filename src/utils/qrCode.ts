...
/**
 * Creates an in-memory image from an SVG string
 */
function createSvgImage(svgString: string): Promise<HTMLImageElement> {
...
export async function generateQrCanvas(
  ...
  // 1. Render base QR code onto canvas with errorCorrectionLevel 'H' (30%)
...
export async function generateQrSvg(
  ...
  const center = qrGridSize / 2;
  const badgeRadius = qrGridSize * 0.115;
  const logoSize = qrGridSize * 0.15;
  const scale = logoSize / 578;
  const logoOffset = center - logoSize / 2;

  // Center elements: white circle cushion + black BERVOS logo
  const centerOverlay = `
    <!-- Center white cushion -->
    <circle cx="${center}" cy="${center}" r="${badgeRadius}" fill="#ffffff" stroke="#ffffff" stroke-width="${qrGridSize * 0.005}" />
    <!-- Centered black BERVOS logo -->
    <g transform="translate(${logoOffset}, ${logoOffset}) scale(${scale})">
      <path d="${BERVOS_LOGO_PATH}" fill="#000000" />
    </g>
  </svg>`;

  return rawSvg.replace('</svg>', centerOverlay);
}

/**
 * Triggers a web-optimised PNG download of the QR code (1024x1024px)
 */
export async function downloadQrPng(
  url: string,
  filename = 'bervos-qr.png',
  options: GenerateQrCodeOptions = {}
): Promise<void> {
  const canvas = await generateQrCanvas(url, { size: 1024, ...options });
  canvas.toBlob((blob) => {
    if (!blob) return;
...
/**
 * Triggers a scalable SVG download of the QR code
 */
export async function downloadQrSvg(
  url: string,
  filename = 'bervos-qr.svg',
  options: GenerateQrCodeOptions = {}
): Promise<void> {
  const svgString = await generateQrSvg(url, options);
