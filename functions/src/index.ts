...
function generateRandomSlug(length = 5): string {
        ...
        try {
          ...
          if (apiRes.ok) {
            ...
            if (file.endsWith('package.json') || file.endsWith('manifest.json')) {
              const data = JSON.parse(text);
              if (data.version) return { version: data.version };
            } else {
              const vMatch = text.match(/^Version:\s*(\S+)/m);
              const dMatch = text.match(/^Date:\s*(\S+)/m);
              if (vMatch) {
                const result: RepoMeta = { version: vMatch[1] };
                if (dMatch) result.releaseDate = new Date(dMatch[1]).toISOString();
                return result;
              }
            }
          }
 ...
 */
function findProjectFolderPath(projectName: string): string | null {
 ...
 */
async function getRepoCollaboratorList(owner: string, repo: string): Promise<Array<{ login: string; email: string; avatarUrl: string; joinedAt?: string }>> {
  ...
  const authorFirstCommit = new Map<string, string>();
  ...
  const storedCollaborators = new Map<string, string>();
  try {
  ...
  const result: Array<{ login: string; email: string; avatarUrl: string; joinedAt: string }> = [];
  ...
  try {
    ...
    if (folderPath && fs.existsSync(path.join(folderPath, '.git'))) {
      const logOut = execSync('git log --format="%aN|%aE|%aI" | sort -u', { cwd: folderPath, encoding: 'utf8', timeout: 1000 }).trim();
      if (logOut) {
        const map = new Map<string, { login: string; email: string; joinedAt: string }>();
...
app.get(['/public-metrics', '/api/public-metrics'], async (req: express.Request, res: express.Response) => {
  try {
    ...
    const formatPublicData = (data: any[]) => data.map((m: any) => ({
      name: m.name,
...
let memoryLogsCache: { timestamp: number; data: any[] } | null = null;

async function getLogsCache(): Promise<{ timestamp: number; data: any[] } | null> {
...
async function saveLogsCache(data: any[]): Promise<void> {
  const timestamp = Date.now();
...
async function fetchAllUsersAggregated(): Promise<Array<{
  ...
  try {
    ...
    for (const collab of rosaCollabs) {
      ...
      if (existing) {
        if (!existing.projects.includes('Rosa')) existing.projects.push('Rosa');
        if (!existing.displayName && collab.login) existing.displayName = collab.login;
        if (!existing.photoURL && collab.avatarUrl) existing.photoURL = collab.avatarUrl;
        if (!existing.projectDetails) existing.projectDetails = {};
        if (!existing.projectDetails['Rosa']) {
          ...
          if (!existing.projectDetails['Rosa'].channels) existing.projectDetails['Rosa'].channels = [];
    ...
    for (const { ref: colRef, sourceLabel } of collections) {
      try {
        ...
        for (const doc of snapshot.docs) {
          ...
          if (rawProject.toLowerCase() === 'rutinas') rawProject = 'Sonder';
...
/**
 * Server-Side Renderer (SSR) for HTML meta tags & link previews
            ...
            if (linkData) {
              ...
              if (destinationUrl) {
                ...
                if (!isPreviewBot && !isPrefetch) {
                  ...
                  linkDoc.ref.update(updates).catch((e: any) => console.warn('[ShortLink] Failed to record click analytics:', e));
                }
...
async function fetchAllLogsAggregated(): Promise<any[]> {
  const initiatives = await getInitiativesFromSchema();
  ...
  return [...userJoinLogs, ...downloadLogs, ...socialLogs].sort((a, b) =>
    parseLogTime(b.timestamp) - parseLogTime(a.timestamp)
  );
}
 ...
 */
app.get('/api/links', authenticateAdmin, async (_req: express.Request, res: express.Response) => {
  try {
    const db = admin.firestore();
 ...
 */
app.post('/api/links', authenticateAdmin, async (req: express.Request, res: express.Response) => {
  try {
    const db = admin.firestore();
    ...
    if (!cleanSlug) {
      ...
      do {
        ...
        attempts++;
      } while ((RESERVED_SLUGS.has(cleanSlug) || (await db.collection('short_links').doc(cleanSlug).get()).exists) && attempts < 10);
    } else {
      if (!SHORT_SLUG_REGEX.test(cleanSlug)) {
        res.status(400).json({
 ...
 */
app.delete('/api/links/:id', authenticateAdmin, async (req: express.Request, res: express.Response) => {
  try {
    const { id } = req.params;
    const cleanId = (id || '').toLowerCase().trim();
    const db = admin.firestore();
 ...
 */
export const updateMetricsCache = functions.pubsub
