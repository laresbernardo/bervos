    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            if (data.status === 'Published' && data.instagram_media_id && !data.instagram_permalink && facebookAccessToken) {
                try {
                    const permalinkRes = await fetch(`https://graph.facebook.com/v19.0/${data.instagram_media_id}?fields=permalink&access_token=${facebookAccessToken}`);
...
async function publishPostToInstagramDirect(postId, hasGeneratedImage = true) {
    ...
    let permalink = null;
    try {
        const permalinkRes = await fetch(`https://graph.facebook.com/v19.0/${publishData.id}?fields=permalink&access_token=${facebookAccessToken}`);
        const permalinkData = await permalinkRes.json();
        if (permalinkRes.ok && permalinkData.permalink) {
 ...
 */
app.delete('/api/social/:id/instagram/schedule', authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const db = admin.firestore();
        const docRef = db.collection('social_posts').doc(id);
...
async function fetchTelemetryLogsForProject(app, projectId, projectName) {
    const db = admin.firestore(app);
    const logs = [];
...
async function fetchTelemetryLogsViaCli(projectId, projectName) {
    const token = await getValidAccessToken();
    if (!token)
        return [];
    const logs = [];
    const collectionsToTry = ['telemetry', 'downloads', 'metrics'];
    const runLogsQuery = async (collectionName) => {
        return new Promise((resolve) => {
            const postData = JSON.stringify({
                structuredQuery: {
                    from: [{ collectionId: collectionName }],
                            ...
                            if (item && item.document) {
                                ...
                                const getVal = (f) => {
                                    ...
                                    if (f.integerValue !== undefined)
                                        return parseInt(f.integerValue, 10);
                                    if (f.doubleValue !== undefined)
                                        return parseFloat(f.doubleValue);
 ...
 */
app.post('/api/links/reset-all', authenticateAdmin, async (_req, res) => {
    try {
        const db = admin.firestore();
        const snapshot = await db.collection('short_links').get();
        const now = new Date().toISOString();
